        ...
        for (const rawItem of pending) {
            ...
            try {
                switch (item.operation) {
                    ...
                    default:
                        throw new Error(
                            `Operação desconhecida: ${item.operation}`
                        );
                }
                ...
                console.log(
                    `Erro ao processar pending id=${item.id} op=${item.operation}:`,
                    errItem
                );
                ...
                if (nextAttempts < MAX_ATTEMPTS) {
                    ...
                    console.warn(
                        `Máx de tentativas atingido para id=${item.id}. Não será reenfileirado.`
                    );
                }
            }
        }
